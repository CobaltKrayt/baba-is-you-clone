...
import baba.logic.{East, North, South, West}
    ...
    checkGame(
      List(
        ...
        TestFrame(3,
          """..OO>.
            |A....."""),
      ), hint = "When moving the baba is extended from the head in the current direction (and he baba is " +
    ...
    checkGame(
      List(
        ...
        TestFrame(5, List(ChangeDir(West())),
          """..OO>.
            |..A...""")
      ), hint = "It should not be possible to “crash into yourself”: If the baba is currently traveling " +
  ...
  test("testQuickSwitch") {
    checkGame(
      List(
        ...
        "before the next step. For example, if the baba came from the west, and the we changed the current " +
